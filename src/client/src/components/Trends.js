...
class Trends extends React.Component {
    ...
    componentDidMount() {
        ...
        if (currentLocation) {
            ...
            const nearbyLocations = this.handleNearbyLocations(currentCountry);
            this.setState({
    ...
    handleNearbyLocations(search) {
        ...
        if (search && searchCountry) {
            return twitterLocations.filter(
                location =>
                    location.country.toLowerCase().indexOf(searchCountry.toLowerCase()) > -1,
            );
        }
        const { currentCountry } = this.state;
        const nearbyLocations = twitterLocations.filter(
            location => location.country.toLowerCase().indexOf(currentCountry.toLowerCase()) > -1,
        );
    ...
    renderNearbyLocations() {
        ...
        return nearbyLocations.slice(0, 10).map((usedLocation, i) => (
            <div className="col-4" key={`col${i}`}>
                <div
                    className={
                        usedLocation.name === locationName
    ...
    renderRecentLocations() {
        ...
        return recentLocations.slice(-6).map((location, i) => (
            <div className="col-4" key={`col${i}`}>
                <div
                    className={
            ...
            if (results.length === 0) {
                return (
                    <div className="trends--searchResultItem">
                        No results were found. Try selecting from a list
                    </div>
                );
            }
            jsx = results.slice(0, 10).map((result, i) => (
                <div
                        ...
                        <div>
                            {recentLocations.length > 0 && (
                                <div className="trends--recentLocationsContainer">
                                    <h2 className="trends--recentLocationsTitle">
                                        Recent Locations
                                    </h2>
                            ...
                            <div className="trends--searchDropdownContainer">
                                ...
                                <div>
                                    ...
                                    >
                                        <option />{" "}
                                        {searchCountry && (
                                            <option value={searchCountry}>All Cities</option>
                                        )}
